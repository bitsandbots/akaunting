<x-layouts.admin>
    ...
    <x-slot name="content">
        <x-form.container>
            <x-form id="journal-entry-form" route="nonprofit.journal-entries.store">
                <x-form.section>
                    <x-slot name="head">
                        <x-form.section.head
                            title="{{ trans('nonprofit::general.basic') }}"
                            description="{{ trans_choice('nonprofit::general.journal_entry', 1) }}"
                        />
                    </x-slot>
                    <x-slot name="body">
                        <x-form.group.date
                        ...
                        <x-form.group.select
                            ...
                            :options="$currencies"
                            selected="{{ old('currency_code', 'USD') }}"
                            required
                        />
                        <x-form.group.text
                            name="currency_rate"
                            label="{{ trans('nonprofit::general.currency_rate') }}"
                            value="{{ old('currency_rate', '1') }}"
                        />
                    </x-slot>
                </x-form.section>
                ...
                <x-form.section>
                    <x-slot name="foot">
                        <x-form.buttons>
                            <x-form.buttons.cancel route="nonprofit.journal-entries.index" />
                            <button type="submit" name="action" value="draft" class="button button-gray">
                                {{ trans('nonprofit::general.save_draft') }}
                            </button>
    ...
    (function() {
        ...
        document.getElementById('save-post-btn').addEventListener('click', function(e) {
            ...
            if (lines.length < 2) {
                e.preventDefault();
                alert('{{ trans('nonprofit::general.min_two_lines') }}');
                return;
            }
